...
create table hongong4(			-- 테이블 생성 
	tinyint_col tinyint ,		-- TININT 	1바이트 	+-128
    smallint_col smallint ,		-- SMALLINT 2바이트 	+-3만정도
    int_col int ,				-- INT 		4바이트 	+-21억정도
    ...
    longblob_col	longblob ,  -- LONGTXET : 1~42억
    float_col		float ,		--  float : 소주점 7자리 표현
    double_col 		double ,	-- double 소수점 15자리 표현
    date_col		date ,		-- date : 날짜 서장 [ YYYY-MM-DD ]
    time_col		time ,		-- TIME : 시간저장  [ HH:MM:SS ]
    detetime_col	datetime	-- datetime : 날짜시간 저장 [ YYYY-MM-DD HH:MM:SS ]
    );
...
create table c_basket(
    ...
    mno int ,			-- 회원번호 
	amount smallint	,	
    -- foreign key ( 현재테이블 fk필드명 ) references pk필드의테이블명( pk 필드명) 
