...
public class JDBC실행 {
	...
	public static void main(String[] args) {
		...
		// JDBC : 자바와 DB 연동
			// 1. 라이브러리 필요 ! [ mysql-connector-java-8.0.30 ]
					// jar : 미리 단들어진 클래스들의 집합 
			// 2. 해당 프로젝트 라이브러리 추가
				// 1. 해당 프로젝트 오른쪽클릭
	...
	try {	
			...
			Class.forName("com.mysql.cj.jdbc.Driver"); // mysql 드라이브 이름
		...
		*/
			...
			ResultSet rs = ps.executeQuery(); // 쿼리[ SQL 결과 ]
				// 결과가 레코드[행] 여러개 존재하기 떄문에 ResultSet 인터페이스 저장
				// 반복문 이용한 레코드 하나씩 출력
						// null -> 검색레코드1 -> 검색레코드2
						// .next() 						: 다른 레코드 호출 [ 다른 레코드가 있으면 true 없으면 false 반환 ]
