SHOW DATABASES;
-- 한줄 주석 처리
-- ctrl + 엔터 : 커서 위치에 있는 한줄 sql 실행
# 한줄 주석
/* 여러줄 주석 */
-- 화면 확대/축소 : ctrl + 힐 / ctrl + (+/-)
...
/*
	컴퓨터 
		1. 0,1 : 기계어 , 2진코드 , 2진수
        2. 자료[데이터] : 01010101 2진코드를 이용한 데이터 저장
        3. 데이터 정리 저장소[표] --> 데이터베이스[DB] : 자료들의 모임
        4. DB 조작 --> DBMS : 데이터베이스[DB] 관리 시스템
	...
	2. 데이블 생성 [ p. 71 ]
		...
		5. 테이블에 데이터 수정하기 [p.78]
			1. MYSQL 
                ...
                2. 테이블에 직접 수정 -> apply
			2. 코드 작성
				UPDATE DB명.테이블명 SET 필드명 = 새로운값 WHERE 조건
...
/*
	...
	1. DDL : Data Definition L : 데이터베이스 정의어 [ 만들기]
		1. CREATE : 데이터베이스 DB,테이블,뷰 등 생성 만들기
			CREATE TABLE 가계부();
		2. ALTER : 데이터베이스 DB , 테이블, 뷰 등 수정 만들기
			ALTER TABLE 가계부( );
	...
	2. DML : Data Manipulation L : 데이터베이스 조작어[ 테이블내 데이터 가공/조작]
		...
		2. INSERT	: 데이터(레코드=행) 추가
			INSERT INTO 테이블명 VALUES( 데이터1,데이터2,테이더3)
		3. UPDATE : 데이터 수정
			UPDATE 테이블명 SET 필드명 = 수정할데이터
		4. DELETE : 데이터(레코드=행) 삭제
			DELETE FROM 테이블명

	3. DCL  : Data Control L : 데이터베이스 제어어 [ DML 에서만 사용 ]
