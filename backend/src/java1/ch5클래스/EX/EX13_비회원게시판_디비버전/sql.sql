 ...
 create table board(
    ...
    b_view int
		-- int : 4 바이트 정수 +-20억
        -- varchar : 문자열 저장가는 가변형 갈이 varchar ( 문자길이 )
 );
 select*from board;
...
-- 7. 개수 : 
	-- count ( 필드명 ) : 해당 필드의 레코드 수 [ null 제외함 ]
	-- conut( * ) 	: 해당 필드의 레코드 수 [ null 포함 ]
select count(*) from board;		-- 전체 레코드[행] 수 == 전체 게시물 수   
		-- 하나의 레코드의 게시물수 표시
    
    -- 8. 특정[ 조건 ] 게시물 조회
		-- select * from board where 조건[필드명==값] : 해당 값이 필드와 같으면
select * from board where b_no = 1;    -- 번호가 변수(입력받은0 같으면
