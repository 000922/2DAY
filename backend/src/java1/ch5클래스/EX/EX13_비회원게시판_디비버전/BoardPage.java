...
public class BoardPage {
	...
	void view( ) {
		...
		while( true ) {
			System.out.println("1.글쓰기 2.글제목 선택 : "); // 안내[출력-print] -> 입력[scanner] -> 저장[변수]
			int btn = scanner.nextInt(); 	// 	int 변수를 쓰는 이유 ???
			
			if( btn == 1 ) {page.regist();}				// 1번 입력하던 등록함수 호출
	...
	void getBoardlist() {
		...
		BoardDto[] boardlist = control.getBoardlist();	// control 에서 모든 게시물의 배열를 요청한다. -> 모든 게시물의 배열을 반환 받는다
		
	//	for( int i = 0 ; i<boardlist.length ; i++) {}
	for( BoardDto dto : boardlist ) {	// 모든 게시물의 배열을 반복문 돌린다 . [ 항상된 for문 ]
	// for ( 배열의자료형 반복변수명 : 배열 )
