...
public class EX5_학생점수관리 {	// class s
	public static void main(String[] args) {	// main s
		...
		int [] scores = null;	// 여러개 점수를 저장할 변수 [ 메모리 할당 전 ]
		...
		while( run ) {	// 무한루프 s	[ 종료조건 : 5 입력했을때 ]
			...
			else if(selectNo == 2) // 2 입력했을때	( 학생수 입력전 실행 불가 0
			{
				for( int i = 0 ; i<scores.length ; i++ ) {	// i는 0부터 배열의 길이까지 1씩증가반복
			...
			else if(selectNo == 3) // 3 입력했을때
			{
				...
				for( int i = 0 ; i<scores.length ; i++ ) {	
					// System.out.printf("scores["+i+"]> , i , "scores[i] );
					System.out.printf("scores[%d]> %d \n" , i , scores[i] );
				}	// for end
			}	
			else if(selectNo == 4) // 4 입력했을
			{
				int max = 0;	// 최고점수
				...
				double avg = (double)sum/scores.length;
								// * 강제형변환 int -> double
				System.out.println("평균 점수 : " + avg );		// 평군 : 합계/개수
				...
				{System.out.println("안내) 알수 없는 번호 입니다.");}	// 그외 입력했을때 
