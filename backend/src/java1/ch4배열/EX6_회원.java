...
public class EX6_회원 {	// class s
	...
	public static void main(String[] args) { // main s
		...
		// 0. 메모리
		String[][] memberlist = new String[100][4]; // 문자열 400개를 저장 할 수 있다.
		...
		while( true ) 	{ // while s	[종료조건 : 없다]
			...
			if( btn == 1 ) 		// 1 입력했을때
				{ 
						...
						else if ( login == 1 ) { System.out.println("안내) 아이디가 일치 하지 않습니다."); }
				
				}
			else if( btn == 2 ) 	// 2 입력했을때
				{ 
					System.out.println("============회원가입 페이지==================");
					// 1. 회원정보 4개의 변수로 입력받기 [ 바로 배열에 대입x ]
					System.out.println("아이디 :");	String id = scanner.next();
					...
					for( int row = 0 ; row<memberlist.length ; row++) {
						if(memberlist[row][0] !=null && memberlist[row][0].equals(id) ) {	
							// 해당 인덱스의 id가 null 이 아니면서  입력받은 id 와 배열내 id 같다 !! [ 중복 찾음 ]	 // null 은 equals 메소드 사용
					...
					// 3. 입력받은 배열에 저장한다. [ 배열호출 : 배열명[행인덱스][열인덱스] ]
						...
						for( int row = 0 ; row<memberlist.length ; row++ ) {
							if( memberlist[row][0] == null) {
								// row번쨰 아이디가 null 이면 = 회원이 없다 !
								// 2-2 : null 행 위치에 입력받은 데이터를 하나씩 대입
								memberlist[0][0] = id;		memberlist[0][1] = password;
								memberlist[0][2] = name;	memberlist[0][3] = phone;
				...
				System.out.println("============아이디찾기 페이지==================");
					// 1. 이름 , 전화번호 입력받기
					System.out.println("아이디 :");	String name = scanner.next();
					System.out.println("전화번호 :");	String phone= scanner.next();
					...
					for( int i = 0 ; i<memberlist.length ; i++ ) {			// 모든 항목 출력
						...
						if  (i == memberlist.length-1) {	// i가 마지막인덱스와 같으면 [ 모두 검사했는데 조건 못찼음
  		...
  		[조건]
   		...
   		[기능]
   			1. 회원가입 기능
   				...
   				2. 배열내 빈 공간의ㅣ 인덱스[null]를 찾아서 해당 인덱스에 회원정보 넣기 
   					String = null
