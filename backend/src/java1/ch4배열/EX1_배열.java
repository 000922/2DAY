...
public class EX1_배열 { // class s
	public static void main(String[] args) {
		...
		//2. 배열 호출
		System.out.println("배열[0] : " + scores[0] ); // 뱌열내 0번 인덱스 호출 -> 83
		System.out.println("배열[1] : " + scores[1] ); // 뱌열내 0번 인덱스 호출 -> 90
		System.out.println("배열[2] : " + scores[2] ); // 뱌열내 0번 인덱스 호출 -> 87
		System.out.println("배열 : " + scores ); 		 	// 배열명 주소[번지] 호출
		// 3. 배열 반복문 활용
		int sum = 0;
		for(int i=0 ; i<scores.length ; i++ ) {	// i는 0배열의 길이 미만 까지 1씩 증가
			sum += scores[i];					// i번째 인덱스의 값 호출해서 sum변수의 누적 더하기
		} // for end
