...
public class EX4_달력 {
		...
		void  run() {
			...
			System.out.println("\n===================================================");
			/////////////////////////////////////////////////////////////////////
			System.out.println("\n ◀이전[1] ▶다음달[2] 검색[3] 일정추가[4] :");
			int btn = scanner.nextInt();
				// 이전달 : 월에서 1 차감 [ 만일 월이 0 이면 월=12 설정 연도 1 차감 ] 
			if( btn == 1 ) {  month--; if( month == 0) { month=12; year--; } }
				// 다음달 : 월에서 1증가	[ 만일 월이 13이면 월=1 설정 연도 1 증가 ]
	...
	void 일정추가() { // 날짜 , 메모 등
		...
		boolean reseult = 달력DAO.getInstance().일정추가( cdate , ccomment );
		if( reseult ) { System.out.println("일정등록성공"); }	
