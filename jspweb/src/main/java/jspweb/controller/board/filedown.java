...
public class filedown extends HttpServlet {
	...
	//	String uploadpath = "C:\\Users\\504\\git\\2DAY\\jspweb\\src\\main\\webapp\\upload\\"+bfile;
		
		String uploadpath = request.getSession().getServletContext().getRealPath("/upload/"+bfile);
		
		// 3. 해당 경로의 파일을 객체화 [ java에서 파일 클래스 = File ]
		File file = new File(uploadpath);	// 해당 경로에 존재하는 파일을 객체화 불로오기 [ 해당 파일을 조작/메소드 ]
		...
		// 5. 파일 전송  [ 외부와 데이터 통신(스트림) = 바이트단위 ]
			// 1. [스트림 객체 생성] 파일의 내용물의 바이트로 모두 읽어온다.
		...
		/*
		  	...
		  	업로드된폴더[윈도우]	------- 스트림[바이트] -------->		자바 --- OutputStream ----> HTTP[웹]
