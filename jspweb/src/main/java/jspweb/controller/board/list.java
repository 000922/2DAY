...
import jspweb.model.dto.BoardDto;
...
public class list extends HttpServlet {
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		boards.put("startbtn", startbtn);		// 3. 버튼의 시작버튼
		boards.put("endbtn", endbtn);			// 4. 버튼의 끝번호
		boards.put("totalsize", totalsize);		// 5. 전체 게시물 수 
			
		// 3. 응답o
		response.setCharacterEncoding("UTF-8"); 
		response.getWriter().print( boards );	// 이거 잘 보자 
		
		/* 나 혼자 메모
		 
		 response.getWriter().print( array ); 에서 위 boards.put("data", array); 로 같이 보내주기 때문에 
		 response.getWriter().print( array ); 가 아닌 response.getWriter().print( boards ); 로 전체를 같이 
		 보내줘야 한다 .
