...
/*
	정규 표현식 : 문자 특정규칫 , 패턴 , 집합 표현할때 사용되는 언어
	...
	* : 앞에 있는 패턴 0개 반복
		...
		(?=.*[0-9]) : 최소 한개 이상의 숫자 영문 		(?=.* \d)
		(?=.*[!@#$%^&*()_]) : 최소 한개 이상의 특수문자 
		
		
	검사 : 정규표현식.test( 데이터 )	: 맞으면 true , false 
	
	1. /^[a-z]{5,20}$/ 									: 소문자/숫자 5~20글자 패턴 
	2. /^[a-zA-Z0-9]{8,20}$/ 							: 영대소문자/숫자 8~20글자 패턴
	3. /^[a-zA-Z가-힣]{2,20}$/							: 이름
	4. /^([0-9]{2,3})-([0-9]{3,4})-([0-9]{3,4})$/		: 전화번호 : 숫자2~3 - 숫자3~4
	5. /^[a-zA-Z0-9+-_.]+@[a-zA-Z0-9-]+\.[a-zA-Z-9-]$/	: 이메일 : 영대소문자/숫자 @ 영대소문자/숫자 . 영대소문자/숫자
...
// 1. col3 div 모두 가져오기
let col3 = document.querySelectorAll('.col3')	// class col3 이면 모두 호출 [ ALL -> 배열 ]
...
sample4_postcode.addEventListener( 'change' , addresscheck )	// change 된 객체의 정보를 addresscheck함수에게 인수
...
/* 
	tag객체명.addEventListener( '이벤트명' , 하수명() )
	tag객체명.addEventListener( '이벤트명' , function(인수){실행코드} )
	tag객체명.addEventListener( '이벤트명' , )

	문자열.indexOf(찾을문자) : 해당 문자열에 찾을문자의 인덱스 
*/
