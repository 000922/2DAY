...
pimg.addEventListener( 'change' , function(e){ // e : 첨부파일	// e : event 객체 ( 이벤트정보[target , data 등])
	// 1. js 파일 클래스 [ FileReader() ]
	let file = new FileReader() // 객체 생성
		console.log( file )
		console.log( e )		// 이벤트 객체
		console.log( e.target )	// 이벤트 발생 시킨 태크 <input type="file"
		console.log( e.target.files[0] )	// 해당 태그의 등록된 파일호출
		
	// 2. 해당 첨부된 파일 경로 알기	[.readAsDataURL(파일) ]
		// e.target.files[0] : 첨부파일의 등록된 이미지 , readAsDataURL() :해당 파일 읽어오기 
	file.readAsDataURL(e.target.files[0] )	
	// 3. 이미지 태그에 첨부된 이미지 대입
	file.onload = function( e ){		// e : 로드 file 의 이벤트 
		document.querySelector('#pimgpre').src = e.target.result		// 서버[사용자의경로X]와 관련없음 
	}
