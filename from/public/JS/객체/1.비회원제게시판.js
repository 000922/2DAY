/*
      ...
      js : id명
      [등록]
      // 1. html input 태그에 입력한 데이터 호출
      // 2. 객체에 담기
      // 3. 글 등록 버튼을 눌렀을때[마다->함수] 1/2 실행한다.
      // 4. 게시물 객체 여러개 저장할 공간 = 배열 사용
      // 5. 글 등록이 완료 될때마다 board객체를 배열에 저장
           //  6. 글 등록했으면 기준 내용을 지우기
      [출력]
      // 1. js시작될때 / 새로운 글이 등록 될때마다 [ 글 출력 ]     
      [보기]
      // 1. 게시물 출력해서 제목을 클릭 할때마다 [ 게시물 보기 ]
         // 1. 내가 클릭한 게시물(???) : 매개변수 전달 [ 클릭한 게시물의 번호(중복 없는 속성 ) ]
 */
 // 4. 여러개 bord 객체를 저장할 배열 선언 
let boardlist = [ ]
...
function 글등록(){
    ...
    let bpassward = document.getElementById('bpassward').value
    let bttitle = document.getElementById('bttitle').value
   ...
   // 5. 객체를 배열에 저장한다.
   boardlist.push( board ) // 배열명.push (데이터 ): 배열에 데이터 추가
   alert('[안내] 글 등록이 되었습니다.')
   // 6. input에 공백 넣기 
   document.getElementById('bwriter').value = ''
   document.getElementById('bpassward').value = ''
...
function 게시물출력(){
	...
	for( let i = 0 ; i<boardlist.length ; i++){
		// i는 0부터 배열의길이 까지 1씩 증가                           // 배열내 i번쨰 객체내 '제목' 키 호출 -> 값 이 나옴
	   html +=  '<tr>'+
					'<td> '+i+' </td> '+
