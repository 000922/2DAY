 ...
 */
let studentlist = [ ]    // *여러개 학생 갹체를 저장하는 배열 [ 함수 밖에 선언한 이유 : 여러 함수들에서 사용 하기 위해서 ]
학생출력() 
   
function 점수등록(){  // 2. 점수 등록 버튼을 눌렀을때 
		// 1. 학생 객체 : html 입력받은 데이터로 선언
		// document.getElementById('name').value : input 에 입력된 무조건 문자열 가져오기
		// number ( document.getElementById('name').value) : input에 입렫된 문자열을 숫자열 변환해서 가져오기
  const student = {
   ...
   if( 이름중복체크(student.name) ) {  // 이름중복체크 함수에서 결과[return]
	  return // return 함수 종료 하는 의미로 return 될 경우에는 아래코드는 실행되지 않는다.
}
    // 2. 객체를 배열에 담기
    studentlist.push( student )
             // 배열명.push(데이터 ) : 배얄에 데이터 추가 
   // 3. 알림 / 확인
...
function 학생출력(){
	...
	let html = '<tr>' +
	  ...
	  // *** 배열내 데이터를 하나씩 꺼내기 -> 객체 정보를 html 화 바꿔주기         
	 for( let i = 0 ; i<studentlist.length ; i++ ){
		...
		html += '<tr>' +
	           ...
	           '</tr>'
	           // 변수는 문자처리 x
	             // 변수는 앞전에 미리 장의된 단어[ 키워드 = 컴퓨터가 알고 있는 단어 ] 
...
function 학생삭제( i ){ // 학생 삭제하는 함수 [ 매개변수 : 삭제할 식별(배열내 저장된 순서번호 = 인덱스)]
