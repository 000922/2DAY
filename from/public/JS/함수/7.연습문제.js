...
const max = function(){
     ...
     for( let i of array ){      // 배열내 요소(항목=값)하나씩 i에 대입 반복
	     if( maxvalue < i ){ maxvalue = i } // i와 값이 maxvalue 보다 크면 maxvalue 에 i 대입
     }// for end
     return maxvalue
...
// 확인문제2 -2 [ 기본 매개변수 함수 선언 -> ... 매개변수 ]
const max2 = function( ...array ){
	let mamaxvalue = array[0]
	for( let i of array ){
		if( mamaxvalue < i ){ maxvalue = i }
	}
	return maxvalue
}
...
const max3 = function(array , ...array2 ){
	 ...
	 let maxvalue = 0 // 최대값 저장하는 변수 
	 
	 if( typeof( array )=='object'  ){   // 배열이면
	     items = array	
	}else if( typeof( array2 ) != 'number') {
		items = array2
	}	
		// typeof(메모리) : 자료형 확인 함수 [ 1. strong 2. number 3. boolean 4. object ]
	// 최대값 구하기 함수 	
	for( let i of items ){
...
console.log( '최대값 구하기 결과3[배열->매개변수4개] : ' + max3(1,2,3,4 ) )
